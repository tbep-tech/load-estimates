...
for(lev in seq_along(levs)){
  ...
  if(lev == 2)
    p <- p %>% 
      layout(
        yaxis = list(title = 'Total Hydro Load (million m3/yr)')
      )
...
```{r}
totab <- hydat %>% 
  select(bay_segment, YEAR, SOURCE, hy_load) %>% 
  pivot_wider(names_from = SOURCE, values_from = hy_load) %>% 
  rowwise() %>% 
  mutate(Total = sum(AD, DPS , IPS, NPS ,GWS, na.rm = T)) %>% 
  ungroup()
```
